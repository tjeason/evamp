...
//  Collision with the spawner object.
void SpawnerObject::CollisionOccurred( SceneObject *object, unsigned long collisionStamp )
{
	...
	// Prevent the spawner's object from being collected by any of the default engine objects. It can only be collected by user defined objects.
	if( object->GetType() == TYPE_SCENE_OBJECT || object->GetType() == TYPE_ANIMATED_OBJECT || object->GetType() == TYPE_SPAWNER_OBJECT )
		return;

	// Make the spawner invisible. This will prevent other objects from colliding with it until it respawns
	SetVisible( false );
